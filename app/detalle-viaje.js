...
const normalizarDeudas = (debts, members = []) => {
  ...
  return lista.map((d, i) => ({
    ...
    deudor: resolverNombre(
      d.from ?? d.deudor ?? d.debtor ?? d.debtorId,
      members,
    ),
    acreedor: resolverNombre(
      d.to ?? d.acreedor ?? d.creditor ?? d.creditorId,
      members,
    ),
...
function DetalleViajeScreen() {
  ...
  const cargar = async () => {
    if (!tripId) {
      setError("No se encontro el viaje.");
      setLoading(false);
      return;
    }
    try {
  ...
  return (
    <SafeAreaView style={s.container}>
      ...
      <View style={s.header}>
        ...
        <Text style={s.headerTitle}>Che-Planner</Text>
        ...
        >
          ...
          <View style={s.viajeHeader}>
            <View style={s.viajeHeaderTop}>
              <Text style={s.viajeNombre} numberOfLines={1}>
                {nombreViaje}
              </Text>
              <View style={[s.statusBadge, !viajeActivo && s.statusCerrado]}>
                <Text style={s.statusText}>
                  {viajeActivo ? "En curso" : "Cerrado"}
                </Text>
              </View>
            </View>
          ...
          <View style={s.totalCard}>
            ...
            <Text style={s.totalMonto}>
              ${Number(totalASaldar).toLocaleString("es-AR")}
            </Text>
              ...
              <View key={d.id} style={s.balanceCard}>
                <View style={s.balanceLeft}>
                  <View style={s.avatarStack}>
                    <View
                      style={[s.miniAvatar, { backgroundColor: "#f4b8b0" }]}
                    >
            ...
            summary.expenses.slice(0, 4).map((e, i) => (
              <View key={e._id || i} style={s.gastoCard}>
                ...
                <View style={s.gastoInfo}>
                  <Text style={s.gastoDesc} numberOfLines={1}>
                    {e.description || "Sin descripcion"}
                  </Text>
                ...
                <View style={s.gastoRight}>
                  <Text style={s.gastoMonto}>
                    ${Number(e.totalAmount || 0).toLocaleString("es-AR")}
                  </Text>
                </View>
              </View>
            ))
          ...
          {viajeActivo && (
            <View style={s.finalizarSection}>
              ...
              >
                <Text style={s.finalizarIcon}>🏁</Text>
                <Text style={s.finalizarText}>
                  {cerrando ? "Cerrando..." : "Finalizar Viaje"}
                </Text>
              </Pressable>
              <Text style={s.finalizarSub}>
                Finalizar el viaje bloqueara la edicion y generara el resumen
                final de pagos.
              </Text>
            </View>
          )}
      ...
      <Modal
        ...
        transparent
        animationType="fade"
        onRequestClose={() => setCerrarModalVisible(false)}
      >
        <View style={s.overlay}>
          <View style={s.modalCard}>
            ...
            {cerrarError ? (
              <Text style={s.modalError}>{cerrarError}</Text>
            ) : null}
            <View style={s.modalBtns}>
              <Pressable
                style={s.cancelBtn}
              ...
              <Pressable
                style={[s.dangerBtn, cerrando && s.disabled]}
                onPress={confirmarCierre}
                disabled={cerrando}
              >
                <Text style={s.dangerText}>
                  {cerrando ? "Cerrando..." : "Finalizar"}
                </Text>
              </Pressable>
            </View>
          </View>
      ...
      <View style={s.nav}>
        ...
        <Pressable
          style={s.navItem}
          onPress={() => router.push(`/deudas?tripId=${tripId}`)}
        >
          <Text style={s.navIcon}>🧾</Text>
        ...
        <Pressable style={s.navItem} onPress={() => router.push("/profile")}>
          <View style={s.navAvatar}>
            <Text style={s.navAvatarText}>{inicialUsuario}</Text>
          </View>
